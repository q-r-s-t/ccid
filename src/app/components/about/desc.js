...
function DescItem({ id, title, description, imageUrl }) {
  ...
  const textStyleKr = "font-[450] leading-[1.3] mt-[-0.3vh] text-[5vw] md:text-[4.5vw] lg:text-[2.2vw]";
  const { lang } = useLanguageStore();
  ...
  return (
    <li className="h-auto bg-gradient-to-t from-[rgba(93,0,156,0.2)] via-[rgba(93,0,156,0)] to-[rgba(93,0,156,0)] lg:flex group overflow-hidden transition-all duration-700 ease-out py-2 lg:py-[2vh] lg:px-[5vw]">
      <div className="w-full lg:w-[52%]">
        <h3
          className={\`\${lang === 'en' ? textStyle : textStyleKr} \${programme.className} transition-all duration-700 tracking-[-0.02em] pt-[2px] mb-[2vh]\`}
        >
          {title}
        </h3>
        <pre className={\`whitespace-pre-wrap \${pxGrotesk.className} pb-[2vh] \${lang === 'en' ? 'leading-[1.38] lg:leading-[1.55] text-[3.8vw] md:text-[2.85vw] lg:text-[1.15vw]' : 'leading-[1.8] text-[3.8vw] lg:text-[1.1vw]'} text-primaryB transition-all duration-700 ml-[4px]\`}>
